...
namespace db
{
    public class QrCodeGenerator
    {
        ...
        private static class ModulePlacer
        {
            ...
            {
                var isBlocked = false;
                foreach (var dummy in blockedModules.Where(blockedMod => Intersects(blockedMod, r1)))
                    isBlocked = true;
                return isBlocked;
            }
        ...
        private void CreateAlignmentPatternTable()
        {
            ...
            for (var i = 0; i < (7 * 40); i = i + 7)
            {
                ...
                for (var x = 0; x < 7; x++)
                {
                    if (_alignmentPatternBaseValues[i + x] == 0) continue;
                    for (var y = 0; y < 7; y++)
                    {
                        if (_alignmentPatternBaseValues[i + y] == 0) continue;
